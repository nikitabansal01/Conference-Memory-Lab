...
Users already brainstorm projects and learning goals in ChatGPT or Claude with **memory on**. This prompt asks the chatbot to **summarize what it already knows about you** into a format you paste into **Your Unique Lens** in Conference Memory Lab.

**Flow:** Copy prompt from the app → paste in ChatGPT/Claude → copy the response → paste into the lens form (or use Import in the app).
...
In Conference Memory Lab → **Your Unique Lens** → **Import from ChatGPT/Claude**, paste the full response and click **Apply to form**. Review each field, edit anything wrong, then **Save lens**.

The app's event loop (Attend → Think → Connect → Create → Review) uses this lens to filter what matters at each event.
